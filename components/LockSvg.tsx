import Svg, { Circle, Path, SvgProps } from 'react-native-svg';

type Props = SvgProps & {
  height: number;
  width: number;
};

export function GameIconsUpgrade({ height, width, ...props }: Props) {
  return (
    <Svg
      width={width}
      height={height}
      fill="none"
      viewBox="0 0 512 512"
      {...props}
    >
      <Path
        fill="currentColor"
        d="m256 29.816l-231 154v106.368l231-154l231 154V183.816zm0 128.043L105 259.783v90.283l151-101.925l151 101.925v-90.283zm0 112l-87 58.725v67.6l87-58l87 58v-67.6zm0 89.957l-87 58v64.368l87-58l87 58v-64.368z"
      ></Path>
    </Svg>
  );
}

export const LockSvg = ({ height, width, ...props }: Props): JSX.Element => {
  return (
    <Svg
      height={height}
      width={width}
      viewBox="0 0 42 43"
      fill="none"
      {...props}
    >
      <Circle cx="21" cy="21.5" r="21" fill="#E7EDFF" />
      <Path
        d="M11 26C11 23.172 11 21.757 11.879 20.879C12.757 20 14.172 20 17 20H25C27.828 20 29.243 20 30.121 20.879C31 21.757 31 23.172 31 26C31 28.828 31 30.243 30.121 31.121C29.243 32 27.828 32 25 32H17C14.172 32 12.757 32 11.879 31.121C11 30.243 11 28.828 11 26Z"
        stroke="#0047FF"
        stroke-width="1.5"
      />
      <Path
        d="M15 20V18C15 16.4087 15.6321 14.8826 16.7574 13.7574C17.8826 12.6321 19.4087 12 21 12C22.5913 12 24.1174 12.6321 25.2426 13.7574C26.3679 14.8826 27 16.4087 27 18V20"
        stroke="#0047FF"
        stroke-width="1.5"
        stroke-linecap="round"
      />
    </Svg>
  );
};

export const HelpSvg = ({ height, width, ...props }: Props): JSX.Element => {
  return (
    <Svg
      height={height}
      width={width}
      viewBox="0 0 42 43"
      fill="none"
      {...props}
    >
      <Circle cx="21" cy="21.5" r="21" fill="#E7EDFF" />
      <Path
        d="M20.6667 11C18.557 11 16.4947 11.6256 14.7406 12.7977C12.9865 13.9697 11.6193 15.6356 10.812 17.5847C10.0046 19.5338 9.79339 21.6785 10.205 23.7476C10.6165 25.8168 11.6324 27.7174 13.1242 29.2091C14.616 30.7009 16.5166 31.7168 18.5857 32.1284C20.6548 32.54 22.7995 32.3287 24.7486 31.5214C26.6977 30.714 28.3636 29.3469 29.5357 27.5927C30.7077 25.8386 31.3333 23.7763 31.3333 21.6667C31.3333 18.8377 30.2095 16.1246 28.2091 14.1242C26.2088 12.1238 23.4956 11 20.6667 11ZM20.6667 31C18.8207 31 17.0162 30.4526 15.4813 29.427C13.9465 28.4015 12.7502 26.9438 12.0438 25.2384C11.3374 23.5329 11.1525 21.6563 11.5127 19.8458C11.8728 18.0353 12.7617 16.3723 14.067 15.067C15.3723 13.7617 17.0353 12.8728 18.8458 12.5127C20.6563 12.1525 22.5329 12.3374 24.2384 13.0438C25.9438 13.7502 27.4015 14.9465 28.4271 16.4813C29.4526 18.0162 30 19.8207 30 21.6667C30 24.142 29.0167 26.516 27.2663 28.2663C25.516 30.0167 23.142 31 20.6667 31Z"
        fill="#0047FF"
      />
      <Path
        d="M20.8597 15.613C20.1372 15.601 19.421 15.7484 18.762 16.0445C18.1029 16.3406 17.5172 16.7782 17.0463 17.3263C16.9809 17.3869 16.9283 17.4601 16.8917 17.5414C16.8551 17.6227 16.8351 17.7105 16.833 17.7996C16.833 17.8819 16.8495 17.9633 16.8815 18.0391C16.9136 18.1148 16.9606 18.1833 17.0197 18.2406C17.0788 18.2978 17.1487 18.3425 17.2255 18.3722C17.3022 18.4018 17.3841 18.4157 17.4663 18.413C17.6425 18.4086 17.8109 18.3398 17.9397 18.2196C18.296 17.8021 18.7385 17.4667 19.2368 17.2365C19.7352 17.0064 20.2774 16.8869 20.8263 16.8863C22.3997 16.8863 23.3797 17.8996 23.3797 19.1596V19.193C23.3797 20.6663 22.2063 21.4863 20.353 21.6263C20.278 21.6292 20.2044 21.6479 20.1371 21.6812C20.0698 21.7145 20.0102 21.7617 19.9624 21.8196C19.9146 21.8775 19.8795 21.9448 19.8595 22.0172C19.8395 22.0896 19.835 22.1654 19.8463 22.2396C19.8463 22.2396 19.8463 23.7863 19.8463 24.073C19.8629 24.238 19.9404 24.3909 20.0637 24.5018C20.187 24.6128 20.3472 24.6738 20.513 24.673H20.5863C20.7514 24.6564 20.9043 24.5789 21.0152 24.4556C21.1262 24.3324 21.1872 24.1722 21.1863 24.0063V22.633C23.1863 22.353 24.8063 21.2996 24.8063 19.113V19.0796C24.7863 17.0796 23.2263 15.613 20.8597 15.613Z"
        fill="#0047FF"
      />
      <Path
        d="M20.5199 27.9665C20.9801 27.9665 21.3532 27.5934 21.3532 27.1331C21.3532 26.6729 20.9801 26.2998 20.5199 26.2998C20.0596 26.2998 19.6865 26.6729 19.6865 27.1331C19.6865 27.5934 20.0596 27.9665 20.5199 27.9665Z"
        fill="#0047FF"
      />
    </Svg>
  );
};
export const LogOutSvg = ({ height, width, ...props }: Props): JSX.Element => {
  return (
    <Svg
      height={height}
      width={width}
      viewBox="0 0 24 24"
      fill="none"
      {...props}
    >
      <Path
        d="M8.00195 7C8.01395 4.825 8.11095 3.647 8.87895 2.879C9.75795 2 11.172 2 14 2H15C17.829 2 19.243 2 20.122 2.879C21 3.757 21 5.172 21 8V16C21 18.828 21 20.243 20.122 21.121C19.242 22 17.829 22 15 22H14C11.172 22 9.75795 22 8.87895 21.121C8.11095 20.353 8.01395 19.175 8.00195 17"
        stroke="#FF0000"
        strokeWidth={1.5}
        strokeLinecap="round"
      />
      <Path
        d="M8 19.5C5.643 19.5 4.464 19.5 3.732 18.768C3 18.035 3 16.857 3 14.5V9.5C3 7.143 3 5.964 3.732 5.232C4.464 4.5 5.643 4.5 8 4.5"
        stroke="#FF0000"
        strokeWidth={1.5}
      />
      <Path
        d="M15 12H6M6 12L8 14M6 12L8 10"
        stroke="#FF0000"
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </Svg>
  );
};

export const UserSvg = ({ height, width, ...props }: Props): JSX.Element => {
  return (
    <Svg
      height={height}
      width={width}
      viewBox="0 0 42 42"
      fill="none"
      {...props}
    >
      <Circle cx="21" cy="21" r="21" fill="#E7EDFF" />
      <Path
        d="M29 31v-2a4 4 0 0 0-4-4H17a4 4 0 0 0-4 4v2"
        stroke="#0047FF"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Circle
        cx="21"
        cy="17"
        r="4"
        stroke="#0047FF"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </Svg>
  );
};
