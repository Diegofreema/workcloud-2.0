...
export const addReview = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const owner = await getUserByIdHelper(ctx, org.ownerId);
    const reviewer = await getUserByIdHelper(ctx, args.userId);
    if (!owner) {
...
export const addReply = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const replyTo = await getUserByIdHelper(ctx, review.userId);
  ...
  handler: async (ctx, { organizationId }) => {
    ...
    return await Promise.all(
      reviews.map(async (review) => {
        const userProfile = await getUserByIdHelper(ctx, review.userId);
        return {
          ...review,
          user: userProfile,
        };
